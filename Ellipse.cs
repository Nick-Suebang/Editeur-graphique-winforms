...
namespace tp_editeur_graphique_winforms_Nick_Suebang
{
    ...
    {
        public Ellipse(Point debut, Point fin, Color couleur, int epaisseur) : base(debut, fin, couleur, epaisseur)
        {
        }
        ...
        public override void Dessiner(Graphics graphe)
        {
            using (Pen pen = new Pen(Couleur, Epaisseur))
            {
                ...
                graphe.DrawEllipse(pen, rect);
            }
        }

        public override string Sauvegarder()
        {
            return $"Ellipse;{Debut.X};{Debut.Y};{Fin.X};{Fin.Y};{Couleur.R};{Couleur.G};{Couleur.B};{Epaisseur}";
        }
    }
}
